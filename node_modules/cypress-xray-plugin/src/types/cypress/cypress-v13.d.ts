...
export type PluginConfig = (on: PluginEvents, config: PluginConfigOptions) => ConfigOptions | Promise<ConfigOptions> | undefined;
...
export type CommandFn<T extends keyof ChainableMethods> = (this: Mocha.Context, ...args: Parameters<ChainableMethods[T]>) => ReturnType<ChainableMethods[T]> | undefined;
export type CommandFns = Record<string, (this: Mocha.Context, ...args: any) => any>;
export type CommandFnWithSubject<T extends keyof ChainableMethods, S> = (this: Mocha.Context, prevSubject: S, ...args: Parameters<ChainableMethods[T]>) => ReturnType<ChainableMethods[T]> | undefined;
export type CommandFnsWithSubject<S> = Record<string, (this: Mocha.Context, prevSubject: S, ...args: any) => any>;
...
export interface CommandOriginalFnWithSubject<T extends keyof ChainableMethods, S> extends CallableFunction {
    (prevSubject: S, ...args: Parameters<ChainableMethods[T]>): ReturnType<ChainableMethods[T]>;
}
export type CommandFnWithOriginalFn<T extends keyof Chainable> = (this: Mocha.Context, originalFn: CommandOriginalFn<T>, ...args: Parameters<ChainableMethods[T]>) => ReturnType<ChainableMethods[T]> | undefined;
export type CommandFnWithOriginalFnAndSubject<T extends keyof Chainable, S> = (this: Mocha.Context, originalFn: CommandOriginalFnWithSubject<T, S>, prevSubject: S, ...args: Parameters<ChainableMethods[T]>) => ReturnType<ChainableMethods[T]> | undefined;
export type QueryFn<T extends keyof ChainableMethods> = (this: Command, ...args: Parameters<ChainableMethods[T]>) => (subject: any) => any;
export type QueryFnWithOriginalFn<T extends keyof Chainable> = (this: Command, originalFn: QueryFn<T>, ...args: Parameters<ChainableMethods[T]>) => (subject: any) => any;
...
export interface Cypress {
    ...
    Commands: {
        ...
        add<T extends keyof Chainable>(name: T, options: CommandOptions & {
            prevSubject: false;
        }, fn: CommandFn<T>): void;
        add<T extends keyof Chainable, S = any>(name: T, options: CommandOptions & {
            prevSubject: true;
        }, fn: CommandFnWithSubject<T, S>): void;
        add<T extends keyof Chainable, S extends PrevSubject>(name: T, options: CommandOptions & {
            prevSubject: ["optional"] | S;
        }, fn: CommandFnWithSubject<T, PrevSubjectMap[S]>): void;
        add<T extends keyof Chainable, S extends PrevSubject>(name: T, options: CommandOptions & {
            prevSubject: S[];
        }, fn: CommandFnWithSubject<T, PrevSubjectMap<void>[S]>): void;
        addAll(fns: CommandFns): void;
        addAll(options: CommandOptions & {
            prevSubject: false;
        }, fns: CommandFns): void;
        addAll<S = any>(options: CommandOptions & {
            prevSubject: true;
        }, fns: CommandFnsWithSubject<S>): void;
        addAll<S extends PrevSubject>(options: CommandOptions & {
            prevSubject: ["optional"] | S;
        }, fns: CommandFnsWithSubject<PrevSubjectMap[S]>): void;
        addAll<S extends PrevSubject>(options: CommandOptions & {
            prevSubject: S[];
        }, fns: CommandFnsWithSubject<PrevSubjectMap<void>[S]>): void;
    ...
    dom: {
        ...
        getElementCoordinatesByPosition(element: HTMLElement | JQuery, position: string): ElementCoordinates;
        getElementCoordinatesByPositionRelativeToXY(element: HTMLElement | JQuery, x: number, y: number): ElementPositioning;
        ...
        unwrap(obj: any): any;
        wrap(wrappingElement_function: ((index: number) => JQuery | string) | Element | JQuery | JQuery.htmlString): JQuery;
    };
    ensure: Ensure;
    env(): ObjectLike;
    env(key: string): any;
...
export type ThenReturn<S, R> = R extends undefined ? Chainable<S> : R extends R | undefined ? Chainable<Exclude<R, undefined> | S> : Chainable<S>;
export interface Chainable<Subject = any> {
    $$<TElement extends Element = HTMLElement>(selector: JQuery.Selector, context?: Document | Element | JQuery): JQuery<TElement>;
    and: Chainer<Subject>;
    ...
    clock(now: Date | number, functions?: ("clearInterval" | "clearTimeout" | "Date" | "setInterval" | "setTimeout")[], options?: Loggable): Chainable<Clock>;
    ...
    contains(content: number | RegExp | string, options?: Partial<Loggable & Timeoutable & CaseMatchable & Shadow>): Chainable<Subject>;
    contains<E extends Node = HTMLElement>(content: number | RegExp | string): Chainable<JQuery<E>>;
    contains<K extends keyof HTMLElementTagNameMap>(selector: K, text: number | RegExp | string, options?: Partial<Loggable & Timeoutable & CaseMatchable & Shadow>): Chainable<JQuery<HTMLElementTagNameMap[K]>>;
    contains<E extends Node = HTMLElement>(selector: string, text: number | RegExp | string, options?: Partial<Loggable & Timeoutable & CaseMatchable & Shadow>): Chainable<JQuery<E>>;
    ...
    each<E extends Node = HTMLElement>(fn: (element: JQuery<E>, index: number, $list: E[]) => void): Chainable<JQuery<E>>;
    ...
    filter<K extends keyof HTMLElementTagNameMap>(selector: K, options?: Partial<Loggable & Timeoutable>): Chainable<JQuery<HTMLElementTagNameMap[K]>>;
    ...
    fixture<Contents = any>(path: string, encoding: Encodings, options?: Partial<Timeoutable>): Chainable<Contents>;
    ...
    invoke<K extends keyof Subject, F extends ((...args: any[]) => any) & Subject[K], R = ReturnType<F>>(functionName: K, ...args: any[]): Chainable<R>;
    invoke<K extends keyof Subject, F extends ((...args: any[]) => any) & Subject[K], R = ReturnType<F>>(options: Partial<Loggable & Timeoutable>, functionName: K, ...args: any[]): Chainable<R>;
    invoke<T extends (...args: any[]) => any>(index: number): Chainable<ReturnType<T>>;
    invoke<T extends (...args: any[]) => any>(options: Partial<Loggable & Timeoutable>, index: number): Chainable<ReturnType<T>>;
    ...
    select(valueOrTextOrIndex: (number | string)[] | number | string, options?: Partial<SelectOptions>): Chainable<Subject>;
    ...
    spread<S extends any[] | boolean | number | object | string>(fn: (...args: any[]) => S): Chainable<S>;
    ...
    then<S extends boolean | number | string>(fn: (this: ObjectLike, currentSubject: Subject) => S): Chainable<S>;
    ...
    then<S extends any[] | boolean | number | object | string>(options: Partial<Timeoutable>, fn: (this: ObjectLike, currentSubject: Subject) => S): Chainable<S>;
...
export type ChainableMethods<Subject = any> = {
    [P in keyof Chainable<Subject>]: Chainable<Subject>[P] extends (...args: any[]) => any ? Chainable<Subject>[P] : never;
};
...
export type experimentalCspAllowedDirectives = "child-src" | "default-src" | "form-action" | "frame-src" | "script-src-elem" | "script-src";
...
export type RetryStrategy = RetryStrategyDetectFlakeAndPassOnThresholdType | RetryStrategyDetectFlakeButAlwaysFailType;
...
export interface ResolvedConfigOptions<ComponentDevServerOpts = any> {
    ...
    responseTimeout: number;
    retries: Nullable<{
        openMode?: Nullable<number>;
        runMode?: Nullable<number>;
    } | number | RetryStrategyWithModeSpecs>;
    screenshotOnRunFailure: boolean;
    screenshotsFolder: false | string;
    scrollBehavior: scrollBehaviorOptions;
    setupNodeEvents: (on: PluginEvents, config: PluginConfigOptions) => PluginConfigOptions | Promise<PluginConfigOptions | undefined> | undefined;
...
export interface SuiteConfigOverrides extends Partial<Pick<ConfigOptions, "animationDistanceThreshold" | "blockHosts" | "defaultCommandTimeout" | "env" | "execTimeout" | "includeShadowDom" | "numTestsKeptInMemory" | "pageLoadTimeout" | "redirectionLimit" | "requestTimeout" | "responseTimeout" | "retries" | "screenshotOnRunFailure" | "scrollBehavior" | "slowTestThreshold" | "taskTimeout" | "viewportHeight" | "viewportWidth" | "waitForAnimations">>, Partial<Pick<ResolvedConfigOptions, "baseUrl" | "testIsolation">> {
...
export type PickConfigOpt<T> = T extends keyof DefineDevServerConfig ? DefineDevServerConfig[T] : any;
...
export interface ResolvedComponentFrameworkDefinition {
    ...
    dependencies: (bundler: "vite" | "webpack", projectPath: string) => Promise<DependencyToInstall[]>;
...
export type ThirdPartyComponentFrameworkDefinition = Pick<ComponentFrameworkDefinition, "dependencies" | "detectors" | "name" | "supportedBundlers" | "type"> & {
    icon?: string;
    type: string;
};
...
export type DevServerFn<ComponentDevServerOpts = any> = (cypressDevServerConfig: DevServerConfig, devServerConfig: ComponentDevServerOpts) => Promise<ResolvedDevServerConfig> | ResolvedDevServerConfig;
...
export interface ComponentConfigOptions<ComponentDevServerOpts = any> extends Omit<CoreConfigOptions, "baseUrl" | "experimentalStudio"> {
    devServer: DevServerConfigOptions | DevServerFn<ComponentDevServerOpts>;
    devServerConfig?: ComponentDevServerOpts;
    experimentalSingleTabRunMode?: boolean;
}
export type UserConfigOptions<ComponentDevServerOpts = any> = Omit<ResolvedConfigOptions<ComponentDevServerOpts>, "baseUrl" | "excludeSpecPattern" | "indexHtmlFile" | "specPattern" | "supportFile">;
export type ConfigOptions<ComponentDevServerOpts = any> = Partial<UserConfigOptions<ComponentDevServerOpts>> & {
    hosts?: null | Record<string, string>;
};
...
export interface Chainer<Subject> {
    ...
    (chainer: "have.all.deep.keys" | "have.all.keys" | "have.deep.keys" | "have.keys", ...value: string[]): Chainable<Subject>;
    ...
    (chainer: "have.length.above" | "have.length.gt" | "have.lengthOf.above" | "have.lengthOf.gt", value: number): Chainable<Subject>;
    (chainer: "have.length.at.least" | "have.length.gte" | "have.lengthOf.at.least" | "have.lengthOf.gte", value: number): Chainable<Subject>;
    (chainer: "have.length.lessThan" | "have.lengthOf.lessThan", value: number): Chainable<Subject>;
    (chainer: "have.length.below" | "have.length.lt" | "have.lengthOf.below" | "have.lengthOf.lt", value: number): Chainable<Subject>;
    (chainer: "have.length.at.most" | "have.length.lte" | "have.lengthOf.at.most" | "have.lengthOf.lte", value: number): Chainable<Subject>;
    ...
    (chainer: "have.a.property" | "have.deep.nested.property" | "have.deep.own.property" | "have.deep.property" | "have.nested.property" | "have.own.property" | "have.property", property: string, value?: any): Chainable<Subject>;
    ...
    (chainer: "deep.include" | "deep.nested.include" | "deep.own.include" | "include" | "nested.include" | "own.include", value: any): Chainable<Subject>;
    (chainer: "include.deep.ordered.members" | "include.members" | "include.ordered.members", value: any[]): Chainable<Subject>;
    ...
    (chainer: "throw", error: ((...args: unknown[]) => unknown) | Error, expected?: RegExp | string): Chainable<Subject>;
    ...
    (chainer: "not.have.all.deep.keys" | "not.have.all.keys" | "not.have.deep.keys" | "not.have.keys", ...value: string[]): Chainable<Subject>;
    ...
    (chainer: "not.have.length.above" | "not.have.length.gt" | "not.have.lengthOf.above" | "not.have.lengthOf.gt", value: number): Chainable<Subject>;
    (chainer: "not.have.length.at.least" | "not.have.length.gte" | "not.have.lengthOf.at.least" | "not.have.lengthOf.gte", value: number): Chainable<Subject>;
    (chainer: "not.have.length.lessThan" | "not.have.lengthOf.lessThan", value: number): Chainable<Subject>;
    (chainer: "not.have.length.below" | "not.have.length.lt" | "not.have.lengthOf.below" | "not.have.lengthOf.lt", value: number): Chainable<Subject>;
    (chainer: "not.have.length.at.most" | "not.have.length.lte" | "not.have.lengthOf.at.most" | "not.have.lengthOf.lte", value: number): Chainable<Subject>;
    ...
    (chainer: "not.have.a.property" | "not.have.deep.nested.property" | "not.have.deep.own.property" | "not.have.deep.property" | "not.have.nested.property" | "not.have.own.property" | "not.have.property", property: string, value?: any): Chainable<Subject>;
    ...
    (chainer: "not.deep.include" | "not.deep.nested.include" | "not.deep.own.include" | "not.include" | "not.nested.include" | "not.own.include", value: any): Chainable<Subject>;
    (chainer: "not.include.deep.ordered.members" | "not.include.members" | "not.include.ordered.members", value: any[]): Chainable<Subject>;
    ...
    (chainer: "not.throw", error: ((...args: unknown[]) => unknown) | Error, expected?: RegExp | string): Chainable<Subject>;
    ...
    (chainer: "always.have.been.calledWithMatch" | "be.always.calledWithMatch", ...args: any[]): Chainable<Subject>;
    ...
    (chainer: "always.have.been.calledWith" | "be.always.calledWith", ...args: any[]): Chainable<Subject>;
    ...
    (chainer: "not.always.have.been.calledWithNew" | "not.be.always.calledWithNew"): Chainable<Subject>;
    (chainer: "not.always.have.been.calledWithMatch" | "not.be.always.calledWithMatch", ...args: any[]): Chainable<Subject>;
    ...
    (chainer: "not.always.have.been.calledOn" | "not.be.always.calledOn", obj: any): Chainable<Subject>;
    (chainer: "not.be.calledWith" | "not.have.been.calledWith", ...args: any[]): Chainable<Subject>;
    (chainer: "not.always.have.been.calledWith" | "not.be.always.calledWith", ...args: any[]): Chainable<Subject>;
...
export interface PluginEvents {
    (action: "after:browser:launch", fn: (browser: Browser, browserLaunchDetails: AfterBrowserLaunchDetails) => Promise<void> | undefined): void;
    (action: "after:run", fn: (results: CypressFailedRunResult | CypressRunResult) => Promise<void> | undefined): void;
    (action: "after:screenshot", fn: (details: ScreenshotDetails) => AfterScreenshotReturnObject | Promise<AfterScreenshotReturnObject> | undefined): void;
    ...
    (action: "before:browser:launch", fn: (browser: Browser, afterBrowserLaunchOptions: BeforeBrowserLaunchOptions) => BeforeBrowserLaunchOptions | Promise<BeforeBrowserLaunchOptions> | Promise<void> | undefined): void;
...
export interface Actions {
    (action: "uncaught:exception", fn: (error: Error, runnable: Mocha.Runnable, promise?: Promise<any>) => false | undefined): Cypress;
    (action: "window:confirm", fn: ((text: string) => false | undefined) | SinonSpyAgent<sinon.SinonSpy> | SinonSpyAgent<sinon.SinonStub>): Cypress;
    ...
    (action: "test:before:run:async", fn: (attributes: ObjectLike, test: Mocha.Test) => Promise<any> | undefined): Cypress;
...
export type TypedArray = Float32Array | Float64Array | Int16Array | Int32Array | Int8Array | Uint16Array | Uint32Array | Uint8Array | Uint8ClampedArray;
...
export type Encodings = "ascii" | "base64" | "binary" | "hex" | "latin1" | "ucs-2" | "ucs2" | "utf-16le" | "utf-8" | "utf16le" | "utf8" | null;
export type PositionType = "bottom" | "bottomLeft" | "bottomRight" | "center" | "left" | "right" | "top" | "topLeft" | "topRight";
export type ViewportPreset = "ipad-2" | "ipad-mini" | "iphone-3" | "iphone-4" | "iphone-5" | "iphone-6" | "iphone-6+" | "iphone-7" | "iphone-8" | "iphone-se2" | "iphone-x" | "iphone-xr" | "macbook-11" | "macbook-13" | "macbook-15" | "macbook-16" | "samsung-note9" | "samsung-s10";
